...
function renderDashboard() {
  ...
  setEl('qs-idioms', getIdiomsAndPhrasal().length + '+');
...
function normalizeDisplayText(text) {
  ...
  if (!/[ÃÂáâ€˜€™“”•–—‹›œžŸ]/.test(text)) return text;
  ...
  try {
    const bytes = Uint8Array.from([...text].map(ch => {
      ...
      return reverse1252.get(code) ?? (code & 0xFF);
    }));
...
function getIdiomsAndPhrasal() {
  const idioms = (typeof IDIOMS_PHRASAL !== 'undefined' ? IDIOMS_PHRASAL : [])
    .filter(item => !item.expression.startsWith('idiom_or_phrasal_'))
    .map(item => ({
      ...item,
      meaning: normalizeDisplayText(item.meaning),
      amharic: normalizeDisplayText(item.amharic),
      example: normalizeDisplayText(item.example),
      example2: normalizeDisplayText(item.example2),
    }));

  const phrasalItems = (typeof PHRASAL !== 'undefined' ? PHRASAL : []).map((item, index) => ({
    id: `pv-${index + 1}`,
    expression: item.pv,
    type: 'phrasal',
    meaning: normalizeDisplayText(item.meaning),
    amharic: normalizeDisplayText(item.am || ''),
    example: normalizeDisplayText((item.examples && item.examples[0]) || ''),
    example2: normalizeDisplayText((item.examples && item.examples[1]) || ''),
    category: 'phrasal'
  }));

  const merged = [...idioms, ...phrasalItems];
  const unique = new Map();

  for (const item of merged) {
    const key = `${item.type}:${item.expression.toLowerCase()}`;
    unique.set(key, item);
  }
...
function renderIdioms() {
  ...
  let items = getIdiomsAndPhrasal().filter(item => {
...
function renderSentenceItems() {
  ...
  container.innerHTML = groups.map(cat => {
    ...
    const items = SENTENCES[cat].map(s => {
      const english = normalizeDisplayText(s.en);
      const amharic = normalizeDisplayText(s.am || '');
      const usage = normalizeDisplayText(s.use);
      return `
      <div class="sent-card">
        <div class="sent-en">${english}</div>
        ${amharic ? `<div class="sent-am"><span class="sent-am-flag">🇪🇹</span>${amharic}</div>` : ''}
        <div class="sent-use">📌 ${usage}</div>
      </div>`;
    }).join('');
    return `
